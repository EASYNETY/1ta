...
import { getMockCourses, getMockCourseBySlug, createMockCheckoutSession } from "@/data/mock-course-data"
...
console.log(`%cAPI Client Mode: ${IS_LIVE_API ? "LIVE" : "MOCK"}`, "color: cyan; font-weight: bold;")
...
async function apiClient<T>(endpoint: string, options: FetchOptions = {}): Promise<T> {
  ...
  if (requiresAuth) {
    try {
      ...
      console.warn("Warning: Unable to access token during SSR, skipping auth header.")
    }
  }
  ...
  // --- MOCK Handling ---
  if (!IS_LIVE_API) {
    console.log(`%cAPI Client: Using MOCK for ${options.method || "GET"} ${endpoint}`, "color: orange;")
    return handleMockRequest<T>(endpoint, options)
  }

  // --- LIVE Handling ---
  try {
    console.log(`%cAPI Client: LIVE ${config.method || "GET"} ${API_BASE_URL}${endpoint}`, "color: lightblue;")
...
async function handleMockRequest<T>(endpoint: string, options: FetchOptions): Promise<T> {
  await new Promise((resolve) => setTimeout(resolve, Math.random() * 300 + 100)) // simulate network delay
  ...
  if (endpoint === "/auth/forgot-password" && method === "post") {
    if (!body?.email) throw new Error("Mock API Error: Missing email in forgot-password")
    return mockForgotPassword(body) as unknown as T
  }
  if (endpoint === "/auth/reset-password" && method === "post") {
    if (!body?.token || !body?.password) throw new Error("Mock API Error: Missing token or password in reset-password")
  ...
  console.error(`Mock API: Endpoint "${endpoint}" (Method: ${method}) not implemented`)
  throw new Error(`Mock API: Endpoint "${endpoint}" (Method: ${method}) not implemented`)
}
