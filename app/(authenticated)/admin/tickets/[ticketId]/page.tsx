...
export default function AdminTicketDetailPage() {
    ...
    useEffect(() => {
        // Always attempt to fetch the ticket for the authenticated user.
        // The backend should enforce access control (owner vs staff). This lets ticket owners (students)
        // view their own tickets while still allowing admins/customer care staff to fetch any ticket.
        if (ticketId && user?.id) {
    ...
    // Build the main page content once (ticket exists here)
    const pageContent = (
    ...
    const isOwner = Boolean(user && ticket && ticket.user && user.id === ticket.user.id);

    // If the current user is the ticket owner, render page content directly. Otherwise enforce staff permissions.
    if (isOwner) {
        return pageContent;
    }
