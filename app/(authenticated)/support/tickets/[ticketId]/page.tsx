...
// Note: support ticket page is for owners — no PermissionGuard here
...
export default function AdminTicketDetailPage() {
    ...
    // Allow any authenticated user to fetch their ticket (owners will be returned by the API)
    useEffect(() => {
        if (ticketId && user?.id) {
    ...
    const isStaff = hasAdminAccess(user ?? undefined) || isCustomerCare(user ?? undefined);
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
                                {isStaff ? (
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Badge variant={getStatusVariant(ticket.status)} className="capitalize">{typeof ticket.status === 'string' ? ticket.status.replace("_", " ") : ticket.status}</Badge>
                                    </div>
                                )}
                            </div>
                        </div>
