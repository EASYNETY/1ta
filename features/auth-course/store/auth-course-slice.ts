...
>("auth_courses/fetchAuthCourses", async (_, { rejectWithValue }) => {
	try {
		...
		// The apiClient's `get` method already handles unwrapping the { success, data } envelope.
		// We can expect a direct array of AuthCourse objects here.
		const courses = await get<AuthCourse[]>("/auth_courses");

		// Final validation to ensure we have an array.
		if (!Array.isArray(courses)) {
			console.error(
				"API did not return an array for /auth_courses:",
				courses
			);
			throw new Error("Invalid data format received from the server.");
		}

		console.log(`✅ Successfully processed ${courses.length} auth courses.`);

		// The CourseCard component handles image URLs, so we don't need to process them here.
		// This keeps the data fetching layer clean and focused.
		return courses;
	} catch (error) {
